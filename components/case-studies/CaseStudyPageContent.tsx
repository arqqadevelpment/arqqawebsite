...
export function CaseStudyPageContent({ study }: { study: PerformanceCaseStudy }) {
  ...
  return (
    <div className="relative">
          ...
          <Reveal delay={0.12}>
            <h1
              ...
              style={{
                ...
                color: "#ffffff",
                /* Only affects studies whose heroHeadline contains a literal
                   newline — a manual break for a specific line arrangement.
                   Everything else keeps wrapping naturally. */
                whiteSpace: "pre-line",
              }}
            >
              {study.heroHeadline}
      ...
      {/* ══ Proof after the Takeaway — same stacked-row treatment as
          socialProof, just placed after "Why It Worked" instead of before it. ══ */}
      {study.sectionMedia?.afterOutcome && study.sectionMedia.afterOutcome.length > 0 ? (
        <section className="relative w-full" style={{ padding: "4rem 1.5rem 5rem" }}>
          <div
            className="relative mx-auto w-full max-w-6xl flex flex-col"
            style={{ gap: "1.5rem" }}
          >
            {study.sectionMedia.afterOutcome.map((src, i) => (
              <Reveal key={src} delay={0.05 + i * 0.05}>
                <div
                  className="relative overflow-hidden rounded-3xl w-full"
                  style={{
